...
# Add new imports for text processing
import nltk
...
# Set NLTK data path
nltk.data.path.append('/Users/shushengye/nltk_data')

# Create output directory if it doesn't exist
OUTPUT_DIR = "analysis_outputs"
os.makedirs(OUTPUT_DIR, exist_ok=True)

# Add after existing OUTPUT_DIR definition
TOPIC_OUTPUT_DIR = "/Users/shushengye/PycharmProjects/pythonProject/analysis_outputs/Quant3_1_CompSt_1/Mod"
...
class TextProcessor:
    """New class to handle text processing and topic modeling functionality"""
    def __init__(self):
        self.stop_words = set(stopwords.words('english'))
        self.petrol_keywords = {
            'companies': [
                'exxon', 'chevron', 'shell', 'bp', 'total', 'aramco', 'petrochina', 'sinopec',
                'conocophillips', 'eni', 'equinor', 'lukoil', 'gazprom', 'petrobras', 'rosneft'
            ],
            'commodities': [
                'oil', 'crude', 'petroleum', 'brent', 'wti', 'light sweet crude',
                'gas', 'fuel', 'diesel', 'gasoline', 'jet fuel', 'kerosene', 'heating oil'
            ],
            'events': [
                'pipeline', 'refinery', 'production', 'reserves', 'drilling', 'terminal', 'tanker', 'rig',
                'exploration', 'extraction', 'processing', 'fracking', 'wellhead'
            ],
            'geopolitical': [
                'sanctions', 'middle east', 'persian gulf', 'strait of hormuz', 'gulf of mexico',
                'trade war', 'embargo', 'blockade', 'conflict', 'civil unrest', 'terrorism'
            ]
        }
    ...
    def create_topic_model(self, texts, num_topics=30):
        """Create LDA topic model from cleaned texts"""
        # Create petroleum dictionary with weights
        petroleum_terms = {}
        weights = {
            'companies': 1.5,
            'commodities': 2.5,
        ...
        for category, terms in self.petrol_keywords.items():
            for term in terms:
                words = term.lower().split()
                if len(words) > 1:
                    petroleum_terms[term.lower()] = weights[category]
                    for word in words:
                        if word not in petroleum_terms:
                            petroleum_terms[word] = weights[category] * 0.5
                else:
                    petroleum_terms[term.lower()] = weights[category]

        # Prepare documents with emphasis on petroleum terms
        processed_texts = []
        for text in texts:
            ...
            for word in words:
                ...
                if word in petroleum_terms:
                    repeats = int(petroleum_terms[word] * 2)
                    emphasized_words.extend([word] * repeats)
            processed_texts.append(emphasized_words)
    ...
    def extract_topic_features(self, lda_model, corpus, dates):
        """Extract topic distribution features for each document"""
...
class DataPreparation:
    ...
    def fetch_news_corpus(self) -> pd.DataFrame:
        """Fetch news corpus data from database"""
        query = """
        SELECT date, corpus
        FROM quant2_2_1_company_news_corpus
        WHERE symbol IN ('petrol','general')
        AND model = 'nltk_3_1'
        AND frequency = 'D'
    ...
    def prepare_topic_features(self, df_news: pd.DataFrame) -> pd.DataFrame:
        """Process news corpus and extract topic features"""
        # Create topic model
        lda_model, corpus, dictionary = self.text_processor.create_topic_model(
            df_news['corpus'].tolist()
        )
        
        # Extract topic features
        topic_features = self.text_processor.extract_topic_features(
            lda_model, 
            corpus, 
            df_news['date']
        )
        
        # Save topic model information to database
        self.save_topic_model_info(lda_model, dictionary)
        
        return topic_features.reset_index()

    def save_topic_model_info(self, lda_model, dictionary):
        """Save topic model information to CSV files only"""
        # Create timestamp directory
        ts = ANALYSIS_TIMESTAMP
        output_dir = os.path.join(TOPIC_OUTPUT_DIR, f"topic_model_{ts}")
        os.makedirs(output_dir, exist_ok=True)

        # Save topic keywords and weights
        topic_data = []
        ...
        topic_file = os.path.join(output_dir, f"{TARGET_ASSET}_topic_words.csv")
        ...
        for token in dictionary.token2id:
            dict_data.append({
                ...
                'doc_frequency': dictionary.dfs[dictionary.token2id[token]]
            })

        # Save dictionary to CSV
        dict_df = pd.DataFrame(dict_data)
        dict_file = os.path.join(output_dir, f"{TARGET_ASSET}_dictionary.csv")
        ...
        metadata = {
            ...
            'alpha': lda_model.alpha.tolist() if hasattr(lda_model.alpha, 'tolist') else lda_model.alpha,
            'beta': lda_model.eta if hasattr(lda_model, 'eta') else 'auto'
        }
        
        meta_df = pd.DataFrame([metadata])
        meta_file = os.path.join(output_dir, f"{TARGET_ASSET}_model_metadata.csv")
        meta_df.to_csv(meta_file, index=False)

        print(f"Saved topic model information to {output_dir}")

    # Remove the _save_topic_model_to_mysql method as it's no longer needed

    def merge_tsn_with_independents(self, df_returns: pd.DataFrame, 
                                  df_kpi: pd.DataFrame,
                                  topic_features: pd.DataFrame = None) -> pd.DataFrame:
        """
        Merge TSN returns with independent variables, economic KPIs, and topic features.
        """
        ...
        if topic_features is not None:
            df_merged = pd.merge(df_merged, topic_features, on='date', how='left')
            # Fill any missing topic values with 0
    ...
    def prepare_merged_data(self) -> pd.DataFrame:
        ...
        df_news = self.fetch_news_corpus()
        topic_features = self.prepare_topic_features(df_news)
        
        # Merge all data
        df_merged = self.merge_tsn_with_independents(
            df_returns_clean, 
            df_kpi,
            topic_features
        )
...
class DimensionReductionAnalysis:
    def __init__(self, use_mysql: bool = True):
        ...
        # Initialize pca_models dictionary
        self.pca_models = {}
        self._target_category = self._determine_target_category()

    def _determine_target_category(self) -> str:
        """Determine which sector/category the target asset belongs to."""
        ...
        commodity_match = df_commodities[df_commodities['symbol'] == TARGET_ASSET]
        if not commodity_match.empty:
            return f"commodity_{commodity_match.iloc[0]['Sub_Category'].lower().replace(' ', '_')}"
        
        # Check if target is a stock
        df_profiles = read_table_as_df('quant1_1_company_profiles', engine)
        stock_match = df_profiles[df_profiles['ticker'] == TARGET_ASSET]
        if not stock_match.empty:
            return f"sector_{stock_match.iloc[0]['sector'].lower().replace(' ', '_')}"
        
        return None

    def _adjust_target_category_weight(self, reduced_data: pd.DataFrame, weight: float = 0.9) -> pd.DataFrame:
        """
        Scale down the PCs belonging to the target asset's sector/category.
        
        Parameters:
        reduced_data: DataFrame containing PCA components
        weight: The weight to apply to the target category PCs (default: 0.9)
        """
        if not self._target_category:
            return reduced_data
            
        target_cols = [col for col in reduced_data.columns 
                      if col.startswith(self._target_category)]
        
        if target_cols:
            reduced_data_adj = reduced_data.copy()
            reduced_data_adj[target_cols] = reduced_data_adj[target_cols] * weight
            return reduced_data_adj
    ...
    def _create_column_groups(self) -> Dict[str, List[str]]:
        ...
        # Initialize groups with topic features group
        groups = {
            'currencies': [col for col in self.data.columns 
                  if any(x in col for x in ['CADUSD', 'NOKUSD', 'RUBUSD', 
                              'SARUSD', 'AEDUSD', 'KWDUSD', 'QARUSD'])],
            'macro': ['GDP_change', 'CPI_change', 
                 'unemploymentRate_change', 'federalFunds_change'],
            'topics': [col for col in self.data.columns if col.startswith('topic_')][:8]  # Limit to 8 topic PCs
        }
    ...
    def reduce_dimensions(self, n_components: Dict[str, int] = None) -> pd.DataFrame:
        ...
        # Apply weight adjustment to target category PCs
        self.reduced_data = self._adjust_target_category_weight(self.reduced_data)
...
class AdvancedTSNAnalysis:
    ...
    def analyze_rolling_windows(self, target_col: str = f'{TARGET_ASSET}_r_10d', 
                              window_size: int = 84, timestamp=None,
        ...
        window_size: Size of rolling window in trading days (84 ≈ 4 months)
        ...
        for start_idx in range(0, total_samples - window_size, 20):  # Step by 20 days
            ...
            top_features = feature_importance[feature_importance != 0].nlargest(8)
