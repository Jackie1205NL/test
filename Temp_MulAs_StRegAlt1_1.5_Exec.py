...
PROJECT_DIR = Path('/Users/shushengye/PycharmProjects/pythonProject/量化策略/Quant2_3_Multi_asset_analysis/StReg1 standard_1_1')
...
trade_module = import_from_file(PROJECT_DIR / 'Temp_MulAs_StRegAlt1_1.5_Trade.py')
model_module = import_from_file(PROJECT_DIR / 'Temp_MulAs_StRegAlt1_1.5_Mod.py')
...
class StrategyExecutor:
    ...
    def update_rolling_metrics(self, current_date):
        ...
        try:
            ...
            # Ensure reduced_data has datetime index
            if not isinstance(self.reduced_data.index, pd.DatetimeIndex):
                self.reduced_data.index = pd.to_datetime(self.reduced_data.index)
            
            # Create a temporary DimensionReductionAnalysis instance for AdvancedTSNAnalysis
            temp_analysis = type('TempAnalysis', (), {
                'data': self.data,
                'reduced_data': self.reduced_data
            })()
            
            # Initialize analysis components with the temporary analysis object
            advanced = AdvancedTSNAnalysis(temp_analysis)
            
            # Update rolling metrics for all horizons with extended date range
            horizons = [
            ...
            for horizon in horizons:
                ...
                advanced.analyze_rolling_windows(
                    ...
                    end_date=current_date  # Pass the current date explicitly
                )
...
if __name__ == "__main__":
    executor = StrategyExecutor(target_asset='FICO')
